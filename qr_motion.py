...
# รับเฉพาะเป้าที่อยู่ไม่ไกลจากกลางภาพมากเกินไป
CENTER_X_TOL = 220

# น้ำหนักการเลือกเป้า
CENTER_AREA_WEIGHT = 0.002     # ตอนยังไม่ lock ให้อันใหญ่ได้เปรียบนิดหน่อย
LOCK_CENTER_WEIGHT = 0.25      # ตอน lock แล้ว ยัง bias เข้าหากลางภาพเล็กน้อย
...
def detect_qr_in_roi(frame, roi_rect, annotated):
    ...
    x1 = max(0, x - ROI_PAD)
    y1 = max(0, y - ROI_PAD)
    x2 = min(W, x + w + ROI_PAD)
    y2 = min(H, y + h + ROI_PAD)
...
def choose_locked_target(candidates):
    ...
    # ยังไม่มี lock -> เลือกอันที่ใกล้กลางภาพที่สุด
    ...
    # มี lock แล้ว -> เอาที่ใกล้ target เดิม + bias เข้ากลางภาพ
...
def detect_black_motion_and_lock(frame, annotated):
    ...
    # ถ้ากรองแล้วไม่เหลือเลย ค่อย fallback ใช้ candidates เดิมแบบไม่กรอง
    if not candidates:
        for c in contours:
            area = cv2.contourArea(c)
            if area < MIN_BLOB_AREA or area > MAX_BLOB_AREA:
